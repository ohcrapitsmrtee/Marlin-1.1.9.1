...
inline void gcode_M17() {
  ...
  static void resume_print(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=ADVANCED_PAUSE_PURGE_LENGTH, const int8_t max_beep_count=0) {
	...
	//mks add start
	do_blocking_move_to_z(current_position[Z_AXIS]+NOZZLE_PARK_Z_HEIGHT);
	//mks add end
...
inline void gcode_M109() {
 ...
 float mks_bed_temp;                //改热床手动与自动M140相差15度
    if (DEBUGGING(DRYRUN)) return;
                                           //改热床手动与自动相差15度开始
  #if 1
  const bool no_wait_for_cooling = parser.seenval('S');
  mks_bed_temp = parser.value_celsius();       //改
  if(mks_bed_temp > BED_MAXTEMP-15){             //改
    mks_bed_temp = BED_MAXTEMP-15;               //改
  }                                              //改
  thermalManager.setTargetBed(mks_bed_temp);
  #else
  ...
  /*
   * inline void gcode_M140() {
 float mks_bed_temp;                //改热床手动与自动相差15度
    if (DEBUGGING(DRYRUN)) return;
                                           //改热床手动与自动相差15度开始
  #if 1
  const bool no_wait_for_cooling = parser.seenval('S');
  mks_bed_temp = parser.value_celsius();
  if(mks_bed_temp > BED_MAXTEMP-15){
    mks_bed_temp = BED_MAXTEMP-15;
  }
  thermalManager.setTargetBed(mks_bed_temp);
  #else
    if (parser.seenval('S')) thermalManager.setTargetBed(parser.value_celsius());
  ...
  inline void gcode_M190() {
    ...
    float mks_bed_temp;                            //改手自M190温差15度
    if (DEBUGGING(DRYRUN)) return;      
  ...
  if(mks_bed_temp > BED_MAXTEMP-15){            //改
    mks_bed_temp = BED_MAXTEMP-15;                //改
     ...
     thermalManager.setTargetBed(mks_bed_temp);  //mks_bed_tem:改
...
inline void gcode_M502() {
   ...
   *
   *  E[distance] - Retract the filament this far   //E[距离]-把灯丝缩到这么远 
   *  Z[distance] - Move the Z axis by this distance//Z[距离]-将Z轴移动此距离 
   * X[position] - Move to this X position, with Y   //X[位置]-移动到此X位置，Y 
   *  Y[position] - Move to this Y position, with X  //Y[位置]-移动到这个Y位置，用X 
   *  U[distance] - Retract distance for removal (manual reload)*U[距离]-拆卸时的缩回距离（手动重新加载） 
   *  L[distance] - Extrude distance for insertion (manual reload)*L[距离]-挤出插入距离（手动重新加载） 
   *  B[count]    - Number of times to beep, -1 for indefinite (if equipped with a buzzer)*B[计数]-蜂鸣次数，-1表示不确定（如果配备蜂鸣器） 
   *  T[toolhead] - Select extruder for filament changeT[工具头]-选择用于更换灯丝的挤出机 
   *
  ...
  inline void gcode_M600() {
    ...
    // Show initial message  显示初始消息 
    #if ENABLED(ULTIPANEL)
      lcd_advanced_pause_show_message(ADVANCED_PAUSE_MESSAGE_INIT, ADVANCED_PAUSE_MODE_PAUSE_PRINT, target_extruder);
    #endif

    #if ENABLED(HOME_BEFORE_FILAMENT_CHANGE)
      // Don't allow filament change without homing first  不允许在未先归位的情况下更换灯丝 
      if (axis_unhomed_error()) home_all_axes();
    #endif

    #if EXTRUDERS > 1
      // Change toolhead if specified   如有规定，更换工具头 
    ...
    #if EXTRUDERS > 1
      // Restore toolhead if it was changed//如果刀头已更改，则还原刀头 
      if (active_extruder_before_filament_change != active_extruder)
        tool_change(active_extruder_before_filament_change, 0, true);
    #endif

    // Resume the print job timer if it was running//如果打印作业计时器正在运行，请恢复它 
    if (job_running) print_job_timer.start();
  }
