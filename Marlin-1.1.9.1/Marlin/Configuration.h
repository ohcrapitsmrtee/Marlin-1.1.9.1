...
#define STRING_CONFIG_H_AUTHOR "(190528, SL-013B)" // Who made the changes.     固件作者信息
#define SHOW_BOOTSCREEN                          // 注释掉，开机不会显示初始画面，直接进入主页
#define STRING_SPLASH_LINE1 SOURCE_CODE_URL//SHORT_BUILD_VERSION // will be shown during bootup in line 1  开机第1行文字，一般版本号
#define STRING_SPLASH_LINE2 WEBSITE_URL         // will be shown during bootup in line 2  开机第2行文字，一般网站
...
/**
 * Prevent extrusion if the temperature is below EXTRUDE_MINTEMP. //如果温度低于挤压温度，则防止挤压。
 * Add M302 to set the minimum extrusion temperature and/or turn  //*添加M302设置最小挤压温度和/或旋转 
 * cold extrusion prevention on and off.             //*冷挤压预防开关。
 *
...
/**
 * Prevent a single extrusion longer than EXTRUDE_MAXLENGTH.    //防止单个挤出超过“挤出最大长度”。 
 * Note: For Bowden Extruders make this large enough to allow load/unload.  //*注：对于波顿挤出机，使其足够大，以允许加载/卸载。 
 */
 ...
 * Default Max Feed Rate (mm/s)   默认的最大进料率(毫米/秒)
 * Override with M203
 *                                      X, Y, Z, E0 [, E1[, E2[, E3[, E4]]]]
 */
#define DEFAULT_MAX_FEEDRATE          { 200, 200, 6, 70 }    //默认速度 (mm/s)
 ...
 *                                      X, Y, Z, E0 [, E1[, E2[, E3[, E4]]]]
 */
#define DEFAULT_MAX_ACCELERATION      { 500, 500, 100, 400 }    //默认最大加速度 (change/s) change = mm/s
 ...
 */
#define DEFAULT_ACCELERATION          500    // X, Y, Z and E acceleration for printing moves  默认的打印加速度
#define DEFAULT_RETRACT_ACCELERATION  500    // E acceleration for retracts    默认的退加料速度
#define DEFAULT_TRAVEL_ACCELERATION   500    // X, Y, Z acceleration for travel (non printing) movesX,Y,Z 非打印时的默认加速度
 ...
 */
 /*    断料检测配置    */
#define FILAMENT_RUNOUT_SENSOR        //去掉注释，即可开启缺料检测功能。一般使用光电限位开关装在送丝机耗材入口处，高电平表示正常送丝，低电平表示缺料。
#if ENABLED(FILAMENT_RUNOUT_SENSOR)
  #define NUM_RUNOUT_SENSORS   1     // Number of sensors, up to one per extruder. Define a FIL_RUNOUT#_PIN for each.
  #define FIL_RUNOUT_INVERTING false // set to true to invert the logic of the sensor.设置为 true 可反转断料检测开关信号
  #define FIL_RUNOUT_PULLUP          // Use internal pullup for filament runout pins.
  #define FILAMENT_RUNOUT_SCRIPT "M600"   //设置缺料检测激活时运行的脚本，一般为 M600，使打印机暂时打印，换好耗材后，可继续打印。
#endif

//#define MKS_CHANGE_FILAMENT
#if ENABLED(MKS_CHANGE_FILAMENT)
  #define MKS_CHANGE_FILAMENT_XY_POSITION "G1 X10 Y10"   
  #define MKS_CHANGE_FILAMENT_LENGTH  "G1 E-30 F1000"  
#endif
...
//#define DEBUG_LEVELING_FEATURE
  ...
  #define MESH_INSET 10              // Set Mesh bounds as an inset region of the bed   热床校平区域
  ...
  #define MESH_INSET 85//10          // Set Mesh bounds as an inset region of the bed热床校平区域
...
#define NOZZLE_PARK_FEATURE
  ...
  #define NOZZLE_PARK_Z_HEIGHT 2  //Lift or Fall NOZZLE_PARK_Z_HEIGHT mm 
...
//  覆盖默认编码器脉冲数，产生下一个步骤，对于高分辨编码器应该增加，（下面数原始数：4  ，改为7， 先前出现按暂停，变为停止）
