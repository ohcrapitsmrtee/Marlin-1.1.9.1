...
/**
 * Jerk controlled movements planner added Apr 2018 by Eduardo Jos茅 Tagle.
 * Equations based on Synthethos TinyG2 sources, but the fixed-point
 * implementation is new, as we are running the ISR with a variable period.
 * Also implemented the B茅zier velocity curve evaluation in ARM assembler,
...
uint32_t Stepper::advance_dividend[XYZE] = { 0 },
  ...
  int32_t __attribute__((used)) Stepper::bezier_A __asm__("bezier_A");    // A coefficient in B茅zier speed curve with alias for assembler
  int32_t __attribute__((used)) Stepper::bezier_B __asm__("bezier_B");    // B coefficient in B茅zier speed curve with alias for assembler
  int32_t __attribute__((used)) Stepper::bezier_C __asm__("bezier_C");    // C coefficient in B茅zier speed curve with alias for assembler
  uint32_t __attribute__((used)) Stepper::bezier_F __asm__("bezier_F");   // F coefficient in B茅zier speed curve with alias for assembler
  uint32_t __attribute__((used)) Stepper::bezier_AV __asm__("bezier_AV"); // AV coefficient in B茅zier speed curve with alias for assembler
  bool __attribute__((used)) Stepper::A_negative __asm__("A_negative");   // If A coefficient was negative
  bool Stepper::bezier_2nd_half;    // =false If B茅zier curve has been initialized or not
#endif
...
void Stepper::set_directions() {
...
#if ENABLED(S_CURVE_ACCELERATION)
  /**
   *  This uses a quintic (fifth-degree) B茅zier polynomial for the velocity curve, giving
   *  a "linear pop" velocity curve; with pop being the sixth derivative of position:
   *  velocity - 1st, acceleration - 2nd, jerk - 3rd, snap - 4th, crackle - 5th, pop - 6th
   *
   *  The B茅zier curve takes the form:
   *
   ...
   *  through t of the B茅zier form of V(t), we can determine that:
   *
   ...
   *  the B茅zier curve at each point:
   *
   *        V_f(t) = A*t^5 + B*t^4 + C*t^3 + F          [0 <= t <= 1]
   *
   * Floating point arithmetic execution time cost is prohibitive, so we will transform the math to
   * use fixed point values to be able to evaluate it in realtime. Assuming a maximum of 250000 steps
   * per second (driver pulses should at least be 2碌S hi/2碌S lo), and allocating 2 bits to avoid
   * overflows on the evaluation of the B茅zier curve, means we can use
   *
   ...
   *
   *  For AVR, the precision of coefficients is scaled so the B茅zier curve can be evaluated in real-time:
   ...
   *       // Calculate the B茅zier coefficients
   *       if (v1 < v0) {
   *         A_negative = true;
   *         bezier_A = 6 * (v0 - v1);
   *         bezier_B = 15 * (v0 - v1);
...
void Stepper::isr() {
  ...
  do {
     ...
     * Assuming at least 6碌s between calls to this ISR...
     * On AVR the ISR epilogue+prologue is estimated at 100 instructions - Give 8碌s as margin
     * On ARM the ISR epilogue+prologue is estimated at 20 instructions - Give 1碌s as margin
     */
    ...
    if (step_events_completed >= step_event_count) {
      ...
      // Are we in Deceleration phase ?
        ...
        uint32_t step_rate;
          ...
          if (!bezier_2nd_half) {
            // Initialize the B茅zier speed curve
            _calc_bezier_curve_coeffs(current_block->cruise_rate, current_block->final_rate, current_block->deceleration_time_inverse);
            bezier_2nd_half = true;
            // The first point starts at cruise rate. Just save evaluation of the B茅zier curve
