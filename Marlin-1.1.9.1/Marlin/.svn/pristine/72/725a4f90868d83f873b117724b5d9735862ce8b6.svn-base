...
#if DISABLED(DISPLAY_CHARSET_ISO10646_1) && DISABLED(MAPPER_C2C3) && ENABLED(USE_BIG_EDIT_FONT)
...
void lcd_printPGM_utf(const char *str, uint8_t n=LCD_WIDTH) {
  ...
  void lcd_bootscreen() {
    ...
    const uint8_t width = u8g.getWidth(), height = u8g.getHeight(),clo_offx=2,
                  offx = (width - (START_BMPWIDTH)) / 2;//clo_offx for STRING_SPLASH_LINE1 display point
    ...
    do {
      ...
      lcd_setFont(FONT_MENU);
        ...
        u8g.drawStr(txt1X, clo_offx + height - (DOG_CHAR_HEIGHT) * 3 / 2, STRING_SPLASH_LINE1);
