...
void CardReader::checkautostart() {
  ...
  if (cardOK
    #if ENABLED(POWER_LOSS_RECOVERY)
      && !jobRecoverFileExists(0) && !jobRecoverFileExists(1)// Don't run auto#.g when a resume file exists
    #endif
  ) {
    char autoname[10];
...
void CardReader::printingHasFinished() {
  ...
  if (file_subcall_ctr > 0) { // Heading up to a parent file that called current as a procedure.
    ...
    #if ENABLED(POWER_LOSS_RECOVERY)
      removeJobRecoveryFile(0);
       removeJobRecoveryFile(1);
    #endif
  ...
  char job_recovery_file_name0[5] = "bin0";
  char job_recovery_file_name1[5] = "bin1";

  void CardReader::openJobRecoveryFile(uint8_t index, const bool read) {
    if (!cardOK) return;

	if(index == 0)
	{
	    if (jobRecoveryFile0.isOpen()) return;
	    if (!jobRecoveryFile0.open(&root, job_recovery_file_name0, read ? O_READ : O_CREAT | O_WRITE | O_TRUNC | O_SYNC)) {
	      //SERIAL_PROTOCOLPAIR(MSG_SD_OPEN_FILE_FAIL, job_recovery_file_name0);
	      //SERIAL_PROTOCOLCHAR('.');
	      //SERIAL_EOL();
	    }
	  //  else if (!read)
	   //   SERIAL_PROTOCOLLNPAIR(MSG_SD_WRITE_TO_FILE, job_recovery_file_name0);
	}
	else if(index == 1)
	{
	    if (jobRecoveryFile1.isOpen()) return;
	    if (!jobRecoveryFile1.open(&root, job_recovery_file_name1, read ? O_READ : O_CREAT | O_WRITE | O_TRUNC | O_SYNC)) {
	     // SERIAL_PROTOCOLPAIR(MSG_SD_OPEN_FILE_FAIL, job_recovery_file_name1);
	      //SERIAL_PROTOCOLCHAR('.');
	     // SERIAL_EOL();
	    }
	   // else if (!read)
	   //   SERIAL_PROTOCOLLNPAIR(MSG_SD_WRITE_TO_FILE, job_recovery_file_name1);
	}
  }

  void CardReader::closeJobRecoveryFile(uint8_t index) 
  {
  	if(index == 0)
		jobRecoveryFile0.close();
	else if(index == 1)
		jobRecoveryFile1.close();
  }

  bool CardReader::jobRecoverFileExists(uint8_t index) {
    
	if(index == 0)
	{
		const bool exists = jobRecoveryFile0.open(&root, job_recovery_file_name0, O_READ);
   		 if (exists) jobRecoveryFile0.close();
        return exists;
	}
	else if(index == 1)
	{
		const bool exists = jobRecoveryFile1.open(&root, job_recovery_file_name1, O_READ);
   		 if (exists) jobRecoveryFile1.close();
        return exists;
	}
    
  }

  int16_t CardReader::saveJobRecoveryInfo(uint8_t index) {
    if(index == 0)
    {
		jobRecoveryFile0.seekSet(0);
		const int16_t ret = jobRecoveryFile0.write(&job_recovery_info, sizeof(job_recovery_info));
    ...
    {
		jobRecoveryFile1.seekSet(0);
		const int16_t ret = jobRecoveryFile1.write(&job_recovery_info, sizeof(job_recovery_info));
		#if ENABLED(DEBUG_POWER_LOSS_RECOVERY)
		  if (ret == -1) SERIAL_PROTOCOLLNPGM("Power-loss file write failed.");
		#endif
    return ret;
  ...
  int16_t CardReader::loadJobRecoveryInfo(uint8_t index) {
  	if(index == 0)
	{
    		return jobRecoveryFile0.read(&job_recovery_info, sizeof(job_recovery_info));
  	}
	else if(index == 1)
	{
    		return jobRecoveryFile1.read(&job_recovery_info, sizeof(job_recovery_info));
  	}
  }

  

  void CardReader::removeJobRecoveryFile(uint8_t index) {
    job_recovery_info.valid_head = job_recovery_info.valid_foot = job_recovery_commands_count = 0;
    if (jobRecoverFileExists(index)) {
	//SERIAL_PROTOCOLLNPGM("remove1");
      //closefile();
	  if(index == 0)
	  {	//SERIAL_PROTOCOLLNPGM("remove1.0");
      		removeFile(job_recovery_file_name0);
	  }
	  else if(index == 1)
	  {//SERIAL_PROTOCOLLNPGM("remove1.1");
      		removeFile(job_recovery_file_name1);
	  }
      #if ENABLED(DEBUG_POWER_LOSS_RECOVERY)
        SERIAL_PROTOCOLPGM("Power-loss file delete");
        serialprintPGM(jobRecoverFileExists(index) ? PSTR(" failed.\n") : PSTR("d.\n"));
      #endif
    }
  }
