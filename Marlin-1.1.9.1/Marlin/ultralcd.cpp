...
uint16_t max_display_update_time = 0;
  ...
  void menu_action_setting_edit_callback_bool(const char* pstr, bool* ptr, screenFunc_t callbackFunc);
    ...
    void lcd_change_language_menu();
    void lcd_change_language_english();
	void lcd_change_language_chinese();
	void lcd_change_language_Japanese();
  #endif
   ...
   */
   ...
   #ifdef ONLINE_CHANGE_LANGUAGE
   /*
     #define CN_MENU_ITEM(type, label, args...) do { \
    if (_menuLineNr == _thisItemNr) { \
      if (lcdDrawUpdate) \
        lcd_implementation_drawmenu_ ## type(encoderLine == _thisItemNr, _lcdLineNr, label, ## args); \
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_sdcard_pause() {
      ...
      #endif
	  #if ENABLED(MKS_CHANGE_FILAMENT)
        enqueue_and_echo_commands_P(PSTR(MKS_CHANGE_FILAMENT_XY_POSITION));
      #endif
      lcd_reset_status();
    }
  ...
  float mks_current_position[XYZE] = { 0 };
    ...
    static void lcd_power_loss_recovery_cancel() {
      card.removeJobRecoveryFile(0);
      card.removeJobRecoveryFile(1);
    ...
    static void lcd_job_recovery_menu() {
      ...
      STATIC_ITEM_P(msg_power_loss_recovery(), false, false);
      //MENU_ITEM(function, MSG_RESUME_PRINT, lcd_power_loss_recovery_resume);
      CN_MENU_ITEM(function, msg_resume_print(), lcd_power_loss_recovery_resume);
      //MENU_ITEM(function, MSG_STOP_PRINT, lcd_power_loss_recovery_cancel);
      CN_MENU_ITEM(function, msg_stop_print(), lcd_power_loss_recovery_cancel);
      END_MENU();
    }
    ...
    void case_light_menu() {
      ...
      //
      //MENU_BACK(MSG_MAIN);
	  CN_MENU_ITEM(back, msg_main(), lcd_main_menu());
  ...
  void lcd_main_menu() {
    START_MENU();
    //MENU_BACK(MSG_WATCH);
    CN_MENU_ITEM(back, msg_watch(),lcd_status_screen());
    ...
    if (planner.movesplanned() || IS_SD_PRINTING)
      CN_MENU_ITEM(submenu, msg_tune(), lcd_tune_menu);
    else
      //MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);
      CN_MENU_ITEM(submenu, msg_prepare(), lcd_prepare_menu);

    //MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
	CN_MENU_ITEM(submenu, msg_control(), lcd_control_menu);

	#ifdef ONLINE_CHANGE_LANGUAGE
	  CN_MENU_ITEM(submenu, msg_change_language(), lcd_change_language_menu);
	#endif
      ...
      if (card.cardOK) {
        if (card.isFileOpen()) {
          if (card.sdprinting)
            //MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);
            CN_MENU_ITEM(function, msg_pause_print(), lcd_sdcard_pause);
          else
            //MENU_ITEM(function, MSG_RESUME_PRINT, lcd_sdcard_resume);
            CN_MENU_ITEM(function, msg_resume_print(), lcd_sdcard_resume);
          //MENU_ITEM(function, MSG_STOP_PRINT, lcd_sdcard_stop);
          CN_MENU_ITEM(function, msg_stop_print(), lcd_sdcard_stop);
        }
        else {
          //MENU_ITEM(submenu, MSG_CARD_MENU, lcd_sdcard_menu);
          CN_MENU_ITEM(submenu, msg_card_menu(), lcd_sdcard_menu);
          #if !PIN_EXISTS(SD_DETECT)
        ...
        CN_MENU_ITEM(submenu, msg_no_card(), lcd_sdcard_menu);
        #if !PIN_EXISTS(SD_DETECT)
  ...
  void lcd_tune_menu() {
	...
	if(currentfontnameindex ==1) {
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, CN_MSG_NOZZLE, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
	}
	else{
    ...
    #if HAS_HEATED_BED
	if(currentfontnameindex ==1) {
		MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, CN_MSG_BED, &thermalManager.target_temperature_bed, 0, BED_MAXTEMP - 15, watch_temp_callback_bed);
	}
	else{
    ...
    //
      ...
      #if HAS_FAN0
	  if(currentfontnameindex ==1) {
		  MENU_MULTIPLIER_ITEM_EDIT(int3, CN_MSG_FAN_SPEED FAN_SPEED_1_SUFFIX, &fanSpeeds[0], 0, 255);
	  }
	  else{
   ...
   */
    ...
    void lcd_dac_menu() {
	  ...
	  CN_MENU_ITEM(back, msg_control(), lcd_control_menu());
  ...
  void _lcd_preheat(const int16_t endnum, const int16_t temph, const int16_t tempb, const int16_t fan) {
    ...
    void lcd_preheat_m1_menu() {
	  ...
	  CN_MENU_ITEM(back, msg_prepare(), lcd_prepare_menu());
      #if HOTENDS == 1
        #if HAS_HEATED_BED
          //MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0);
          CN_MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0);
          //MENU_ITEM(function, MSG_PREHEAT_1_END, lcd_preheat_m1_e0_only);
          CN_MENU_ITEM(function, msg_preheat_pal_end(), lcd_preheat_m1_e0_only);
        #else
          //MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0_only);
          CN_MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0_only);
        #endif
      #elif HOTENDS > 1
        #if HAS_HEATED_BED
          //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H1, lcd_preheat_m1_e0);
          CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H1, lcd_preheat_m1_e0);
          //MENU_ITEM(function, MSG_PREHEAT_1_END " " MSG_E1, lcd_preheat_m1_e0_only);
          CN_MENU_ITEM(function, msg_preheat_pal_end() " " MSG_E1, lcd_preheat_m1_e0_only);
          //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H2, lcd_preheat_m1_e1);
          CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H2, lcd_preheat_m1_e1);
          //MENU_ITEM(function, MSG_PREHEAT_1_END " " MSG_E2, lcd_preheat_m1_e1_only);
          CN_MENU_ITEM(function, msg_preheat_pal_end() " " MSG_E2, lcd_preheat_m1_e1_only);
        #else
          //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H1, lcd_preheat_m1_e0_only);
          CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H1, lcd_preheat_m1_e0_only);
          //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H2, lcd_preheat_m1_e1_only);
          CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H2, lcd_preheat_m1_e1_only);
        #endif
        #if HOTENDS > 2
          #if HAS_HEATED_BED
            //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H3, lcd_preheat_m1_e2);
            CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H3, lcd_preheat_m1_e2);
            //MENU_ITEM(function, MSG_PREHEAT_1_END " " MSG_E3, lcd_preheat_m1_e2_only);
            CN_MENU_ITEM(function, msg_preheat_pal_end() " " MSG_E3, lcd_preheat_m1_e2_only);
          #else
            //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H3, lcd_preheat_m1_e2_only);
            CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H3, lcd_preheat_m1_e2_only);
          #endif
          #if HOTENDS > 3
            #if HAS_HEATED_BED
              //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H4, lcd_preheat_m1_e3);
              CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H4, lcd_preheat_m1_e3);
              //MENU_ITEM(function, MSG_PREHEAT_1_END " " MSG_E4, lcd_preheat_m1_e3_only);
              CN_MENU_ITEM(function, msg_preheat_pal_end() " " MSG_E4, lcd_preheat_m1_e3_only);
            #else
              //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H4, lcd_preheat_m1_e3_only);
              CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H4, lcd_preheat_m1_e3_only);
            #endif
            #if HOTENDS > 4
              #if HAS_HEATED_BED
                //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H5, lcd_preheat_m1_e4);
                CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H5, lcd_preheat_m1_e4);
                //MENU_ITEM(function, MSG_PREHEAT_1_END " " MSG_E5, lcd_preheat_m1_e4_only);
                CN_MENU_ITEM(function, msg_preheat_pal_end() " " MSG_E5, lcd_preheat_m1_e4_only);
              #else
                //MENU_ITEM(function, MSG_PREHEAT_1_N MSG_H5, lcd_preheat_m1_e4_only);
                CN_MENU_ITEM(function, msg_preheat_pla_n() MSG_H5, lcd_preheat_m1_e4_only);
              #endif
            #endif // HOTENDS > 4
          #endif // HOTENDS > 3
        #endif // HOTENDS > 2
        ...
        CN_MENU_ITEM(function, msg_preheat_pal_bedonly(), lcd_preheat_m1_bedonly);
      #endif
      END_MENU();
    }
    ...
    void lcd_preheat_m2_menu() {
	  ...
	  CN_MENU_ITEM(back, msg_prepare(), lcd_prepare_menu());
          ...
          CN_MENU_ITEM(function, msg_preheat_abs(), lcd_preheat_m2_e0);
          //MENU_ITEM(function, MSG_PREHEAT_2_END, lcd_preheat_m2_e0_only);
          CN_MENU_ITEM(function, msg_preheat_abs_end(), lcd_preheat_m2_e0_only);
        #else
          //MENU_ITEM(function, MSG_PREHEAT_2, lcd_preheat_m2_e0_only);
          CN_MENU_ITEM(function, msg_preheat_abs(), lcd_preheat_m2_e0_only);
        #endif
      #elif HOTENDS > 1
        #if HAS_HEATED_BED
          MENU_ITEM(function, MSG_PREHEAT_2_N MSG_H1, lcd_preheat_m2_e0);
          //MENU_ITEM(function, MSG_PREHEAT_2_END " " MSG_E1, lcd_preheat_m2_e0_only);
          CN_MENU_ITEM(function, msg_preheat_abs_end() " " MSG_E1, lcd_preheat_m2_e0_only);
          MENU_ITEM(function, MSG_PREHEAT_2_N MSG_H2, lcd_preheat_m2_e1);
          //MENU_ITEM(function, MSG_PREHEAT_2_END " " MSG_E2, lcd_preheat_m2_e1_only);
          CN_MENU_ITEM(function, msg_preheat_abs_end() " " MSG_E2, lcd_preheat_m2_e1_only);
        #else
          ...
          MENU_ITEM(function, MSG_PREHEAT_2_N MSG_H2, lcd_preheat_m2_e1_only);
            ...
            CN_MENU_ITEM(function, msg_preheat_abs_end() " " MSG_E3, lcd_preheat_m2_e2_only);
          #else
            MENU_ITEM(function, MSG_PREHEAT_2_N MSG_H3, lcd_preheat_m2_e2_only);
              ...
              CN_MENU_ITEM(function, msg_preheat_abs_end() " " MSG_E4, lcd_preheat_m2_e3_only);
            #else
              MENU_ITEM(function, MSG_PREHEAT_2_N MSG_H4, lcd_preheat_m2_e3_only);
                ...
                CN_MENU_ITEM(function, msg_preheat_abs_end() " " MSG_E5, lcd_preheat_m2_e4_only);
              #else
        ...
        CN_MENU_ITEM(function, msg_preheat_abs_bedonly(), lcd_preheat_m2_bedonly);
      #endif
      END_MENU();
    }
  ...
  void lcd_cooldown() {
    ...
    void _lcd_level_bed_done() {
      ...
      if (lcdDrawUpdate) lcd_implementation_drawedit(msg_level_bed_done(), NULL);
      lcdDrawUpdate = LCDVIEW_CALL_REDRAW_NEXT;
    }
    ...
    void _lcd_level_bed_get_z() {
      ...
      if (lcdDrawUpdate) {
        ...
        lcd_implementation_drawedit(msg_move_z(), ftostr43sign(v + (v < 0 ? -0.0001f : 0.0001f), '+'));
      }
    }
    ...
    void _lcd_level_bed_moving() {
      if (lcdDrawUpdate) {
        ...
        lcd_implementation_drawedit(msg_next(), msg);
      }
    ...
    void _lcd_level_bed_homing_done() {
	  ...
	  #else
		//if(lcdDrawUpdate) lcd_implementation_drawmenu_static(0, msg_level_bed_waiting(), false, false);
		if(lcdDrawUpdate) lcd_implementation_drawedit(msg_level_bed_waiting(), NULL);
	  #endif
      if (use_click()) {
    ...
    #else
	if (lcdDrawUpdate) {
		//lcd_implementation_drawmenu_static(0, msg_level_bed_homing(), false, false);
		lcd_implementation_drawedit(msg_level_bed_homing(), NULL);
		}
    #endif
      lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
    ...
    void _lcd_ubl_height_adjust_menu() {
      ...
      CN_MENU_ITEM(function, msg_watch(), lcd_return_to_status);
      END_MENU();
    }
    ...
    void lcd_bed_leveling() {
      ...
      // Level Bed
        ...
        CN_MENU_ITEM(submenu, msg_bed_leveling(), _lcd_level_bed_continue);
      #else
  ...
  void lcd_prepare_menu() {
    ...
    //
      ...
      if (all_axes_homed())
        ...
        CN_MENU_ITEM(submenu, msg_move_axis(), lcd_move_menu);

    //
    // Auto Home
    //
    //MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
    CN_MENU_ITEM(gcode, msg_auto_home(), PSTR("G28"));
    #if ENABLED(INDIVIDUAL_AXIS_HOMING_MENU)
      //MENU_ITEM(gcode, MSG_AUTO_HOME_X, PSTR("G28 X"));
      CN_MENU_ITEM(gcode, msg_auto_home_x(), PSTR("G28 X"));
      //MENU_ITEM(gcode, MSG_AUTO_HOME_Y, PSTR("G28 Y"));
      CN_MENU_ITEM(gcode, msg_auto_home_y(), PSTR("G28 Y"));
      //MENU_ITEM(gcode, MSG_AUTO_HOME_Z, PSTR("G28 Z"));
      CN_MENU_ITEM(gcode, msg_auto_home_z(), PSTR("G28 Z"));
    #endif
    ...
    //
      ...
      MENU_ITEM(submenu, MSG_UBL_LEVEL_BED, _lcd_ubl_level_bed);
        ...
        if (!g29_in_progress)
          ...
          CN_MENU_ITEM(submenu, msg_bed_leveling(), lcd_bed_leveling);
      ...
      //CN_MENU_ITEM(function, msg_set_home_offsets(), lcd_set_home_offsets);
    #endif
    ...
    CN_MENU_ITEM(gcode, msg_disable_steppers(), PSTR("M84"));
    ...
    //
      ...
      if (!IS_SD_FILE_OPEN) {
          ...
          if (thermalManager.targetHotEnoughToExtrude(active_extruder))
            //MENU_ITEM(gcode, MSG_FILAMENTCHANGE, PSTR("M600 B0"));
            CN_MENU_ITEM(gcode, msg_filamentchange(), PSTR("M600 B0"));
          else
            //MENU_ITEM(submenu, MSG_FILAMENTCHANGE, lcd_temp_menu_e0_filament_change);
            CN_MENU_ITEM(submenu, msg_filamentchange(), lcd_temp_menu_e0_filament_change);
        #else
          //MENU_ITEM(submenu, MSG_FILAMENTCHANGE, lcd_change_filament_menu);
          CN_MENU_ITEM(submenu, msg_filamentchange(), lcd_change_filament_menu);
        #endif
      }
      ...
      //
        ...
        //MENU_ITEM(submenu, msg_preheat_pla(), lcd_preheat_m1_menu);
        CN_MENU_ITEM(submenu, msg_preheat_pla(), lcd_preheat_m1_menu);
        //MENU_ITEM(submenu, MSG_PREHEAT_2, lcd_preheat_m2_menu);
        CN_MENU_ITEM(submenu, msg_preheat_abs(), lcd_preheat_m2_menu);
      #else
        //MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0_only);
        CN_MENU_ITEM(function, msg_preheat_pla(), lcd_preheat_m1_e0_only);
        //MENU_ITEM(function, MSG_PREHEAT_2, lcd_preheat_m2_e0_only);
    ...
    //
      ...
      CN_MENU_ITEM(function, msg_autostart(), lcd_autostart_sd);
    #endif
 ...
 void lcd_move_z() { _lcd_move_xyz(msg_move_z(), Z_AXIS); }
  void _lcd_move_e(
    ...
    if (lcdDrawUpdate) {
      PGM_P pos_label;
      #if E_MANUAL == 1
        pos_label = msg_move_e();//PSTR(MSG_MOVE_E);
      #else
        switch (eindex) {
    ...
    if (LCD_HEIGHT >= 4) {
      switch (axis) {
        case X_AXIS:
          ...
          STATIC_ITEM_P(msg_move_x(), true, true); break;
        case Y_AXIS:
          //STATIC_ITEM(MSG_MOVE_Y, true, true); break;
          STATIC_ITEM_P(msg_move_y(), true, true); break;
        case Z_AXIS:
          //STATIC_ITEM(MSG_MOVE_Z, true, true); break;
          STATIC_ITEM_P(msg_move_z(), true, true); break;
        default:
          //STATIC_ITEM(MSG_MOVE_E, true, true); break;
          STATIC_ITEM_P(msg_move_e(), true, true); break;
      }
    }
    //MENU_BACK(MSG_MOVE_AXIS);
	CN_MENU_ITEM(back, msg_move_axis(), lcd_move_menu);
    //MENU_ITEM(submenu, MSG_MOVE_10MM, lcd_move_menu_10mm);
    CN_MENU_ITEM(submenu, msg_move_10mm(), lcd_move_menu_10mm);
    //MENU_ITEM(submenu, MSG_MOVE_1MM, lcd_move_menu_1mm);
    CN_MENU_ITEM(submenu, msg_move_1mm(), lcd_move_menu_1mm);
    //MENU_ITEM(submenu, MSG_MOVE_01MM, lcd_move_menu_01mm);
    CN_MENU_ITEM(submenu, msg_move_01mm(), lcd_move_menu_01mm);
    END_MENU();
  }
  ...
  void lcd_move_menu() {
    ...
    if (_MOVE_XYZ_ALLOWED) {
      if (_MOVE_XY_ALLOWED) {
        ...
        CN_MENU_ITEM(submenu, msg_move_x(), lcd_move_get_x_amount);
        //MENU_ITEM(submenu, MSG_MOVE_Y, lcd_move_get_y_amount);
        CN_MENU_ITEM(submenu, msg_move_y(), lcd_move_get_y_amount);
      }
      #if ENABLED(DELTA)
        else
      ...
      CN_MENU_ITEM(submenu, msg_move_z(), lcd_move_get_z_amount);
    }
    else
      MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
      ...
      CN_MENU_ITEM(submenu, msg_move_e(), lcd_move_get_e_amount);
      // ...and the non-switching
      #if E_MANUAL == 5
        //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E5, lcd_move_get_e4_amount);
        CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E5, lcd_move_get_e4_amount);
      #elif E_MANUAL == 3
        //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E3, lcd_move_get_e2_amount);
        CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E3, lcd_move_get_e2_amount);
      #endif
      ...
      CN_MENU_ITEM(submenu, msg_move_e(), lcd_move_get_e_amount);
        ...
        CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E1, lcd_move_get_e0_amount);
        //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E2, lcd_move_get_e1_amount);
        CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E2, lcd_move_get_e1_amount);
        #if E_MANUAL > 2
          //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E3, lcd_move_get_e2_amount);
          CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E3, lcd_move_get_e2_amount);
          #if E_MANUAL > 3
            //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E4, lcd_move_get_e3_amount);
            CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E4, lcd_move_get_e3_amount);
            #if E_MANUAL > 4
              //MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E5, lcd_move_get_e4_amount);
              CN_MENU_ITEM(submenu, msg_move_e() MSG_MOVE_E5, lcd_move_get_e4_amount);
            #endif // E_MANUAL > 4
  ...
  void lcd_control_menu() {
    ...
    CN_MENU_ITEM(submenu, msg_temperature(), lcd_control_temperature_menu);
    //MENU_ITEM(submenu, MSG_MOTION, lcd_control_motion_menu);
    CN_MENU_ITEM(submenu, msg_motion(), lcd_control_motion_menu);

    #if DISABLED(NO_VOLUMETRICS) || ENABLED(ADVANCED_PAUSE_FEATURE)
      //MENU_ITEM(submenu, MSG_FILAMENT, lcd_control_filament_menu);
      CN_MENU_ITEM(submenu, msg_volumetric(), lcd_control_filament_menu);
      ...
      CN_MENU_ITEM(submenu, msg_retract(), lcd_control_retract_menu);
    #endif
    #if ENABLED(DAC_STEPPER_CURRENT)
    ...
    #if ENABLED(EEPROM_SETTINGS)
      //MENU_ITEM(function, MSG_STORE_EEPROM, lcd_store_settings);
      CN_MENU_ITEM(function, msg_store_eeprom(), lcd_store_settings);
      //MENU_ITEM(function, MSG_LOAD_EEPROM, lcd_load_settings);
      CN_MENU_ITEM(function, msg_load_eeprom(), lcd_load_settings);
    #endif

    //MENU_ITEM(function, MSG_RESTORE_FAILSAFE, lcd_factory_settings);
    CN_MENU_ITEM(function, msg_restore_failsafe(), lcd_factory_settings);

    #if ENABLED(EEPROM_SETTINGS) && DISABLED(SLIM_LCD_MENUS)
      MENU_ITEM(submenu, MSG_INIT_EEPROM, lcd_init_eeprom_confirm);
      //CN_MENU_ITEM(submenu, MSG_INIT_EEPROM, lcd_init_eeprom_confirm);
    #endif
  ...
  void lcd_control_temperature_menu() {
    ...
    //
      ...
      //
	if(currentfontnameindex ==1) {
		MENU_ITEM(submenu, CN_MSG_PREHEAT_1_SETTINGS, lcd_control_temperature_preheat_material1_settings_menu);
	}
	else{
      ...
      //
	if(currentfontnameindex ==1) {
		MENU_ITEM(submenu, CN_MSG_PREHEAT_2_SETTINGS, lcd_control_temperature_preheat_material2_settings_menu);
	}
	else{
    ...
    void _lcd_control_temperature_preheat_settings_menu(const uint8_t material) {
	  ...
	  if(currentfontnameindex ==1) {
		  MENU_ITEM_EDIT(int3, CN_MSG_FAN_SPEED, &lcd_preheat_fan_speed[material], 0, 255);
	  }
	  else{
	  ...
	  if(currentfontnameindex ==1) {
		  MENU_ITEM_EDIT(int3, CN_MSG_NOZZLE, &lcd_preheat_hotend_temp[material], MINTEMP_ALL, MAXTEMP_ALL - 15);	  	
	  	}
	  else{
      ...
      #if HAS_HEATED_BED
	  if(currentfontnameindex ==1) {
		  MENU_ITEM_EDIT(int3, CN_MSG_BED, &lcd_preheat_bed_temp[material], BED_MINTEMP, BED_MAXTEMP - 15);
	  }
	  else{
   ...
   */
	...
	*/
    void lcd_change_language_menu(){
	  START_MENU();
	  //MENU_BACK(MSG_CHANGE_LANGUAGE);
	  CN_MENU_ITEM(back, msg_change_language(), lcd_main_menu());
	  CN_MENU_ITEM(function, msg_english_lang(), lcd_change_language_english);
	  CN_MENU_ITEM(function, msg_chinese_lang(), lcd_change_language_chinese);
	  //CN_MENU_ITEM(function, msg_japanese_lang(), lcd_change_language_Japanese);
	  END_MENU();
	}
  #endif
      ...
      for (uint16_t i = 0; i < fileCnt; i++) {
        if (_menuLineNr == _thisItemNr) {
          ...
          if (card.filenameIsDir)
            ...
            CN_MENU_ITEM(sddirectory, msg_card_menu(), card);
          else
            //MENU_ITEM(sdfile, MSG_CARD_MENU, card);
            CN_MENU_ITEM(sdfile, msg_card_menu(), card);
        }
        else {
          MENU_ITEM_DUMMY();
        }
...
//          return PSTR(MSG_FILAMENTUNLOAD);
    ...
    void _lcd_temp_menu_filament_op(const AdvancedPauseMode mode, const int8_t extruder) {
	  ...
	  CN_MENU_ITEM(back, msg_filamentchange(), lcd_change_filament_menu);
      //MENU_ITEM(submenu, msg_preheat_pla(), _lcd_change_filament_temp_1_menu);
      CN_MENU_ITEM(submenu, msg_preheat_pla(), _lcd_change_filament_temp_1_menu);
      //MENU_ITEM(submenu, MSG_PREHEAT_2, _lcd_change_filament_temp_2_menu);
      CN_MENU_ITEM(submenu, msg_preheat_abs(), _lcd_change_filament_temp_2_menu);
      END_MENU();
    }
    ...
    static const char* advanced_pause_header() {
      ...
      return msg_filament_change_header_pause();
    }
    ...
    void lcd_advanced_pause_option_menu() {
      START_MENU();
        ...
        STATIC_ITEM_P(msg_filament_change_option_header(), true, false);
      #endif
      //MENU_ITEM(function, MSG_FILAMENT_CHANGE_OPTION_RESUME, lcd_advanced_pause_resume_print);
      CN_MENU_ITEM(function, msg_filament_change_option_resume(), lcd_advanced_pause_resume_print);
      //MENU_ITEM(function, MSG_FILAMENT_CHANGE_OPTION_PURGE, lcd_advanced_pause_extrude_more);
      CN_MENU_ITEM(function, msg_filament_change_option_purge(), lcd_advanced_pause_extrude_more);
      END_MENU();
    }

    void lcd_advanced_pause_init_message() {
      START_SCREEN();
      STATIC_ITEM_P(advanced_pause_header(), true, false);
    ...
    void lcd_advanced_pause_purge_message() {
      ...
      STATIC_ITEM_P(msg_filament_change_purge_1());
      #ifdef MSG_FILAMENT_CHANGE_PURGE_2
        //STATIC_ITEM(MSG_FILAMENT_CHANGE_PURGE_2);
        STATIC_ITEM_P(msg_filament_change_purge_2());
    ...
    void lcd_advanced_pause_resume_message() {
      START_SCREEN();
      STATIC_ITEM_P(advanced_pause_header(), true, false);
      STATIC_ITEM_P(msg_filament_change_resume_1(), true, false);
      #ifdef MSG_FILAMENT_CHANGE_RESUME_2
       // STATIC_ITEM(MSG_FILAMENT_CHANGE_RESUME_2);
       STATIC_ITEM_P(msg_filament_change_resume_2(), true, false);
      #endif
...
#if ENABLED(MKS_CHANGE_FILAMENT)
void mks_lcd_change_header_unload(){
  enqueue_and_echo_commands_now_P(PSTR("G91"));
  enqueue_and_echo_commands_now_P(PSTR(MKS_CHANGE_FILAMENT_LENGTH));
  enqueue_and_echo_commands_now_P(PSTR("G90"));
}
void mks_lcd_change_filament(){
  START_MENU();
  MENU_BACK(MSG_WATCH);
//  MENU_ITEM(function, MSG_FILAMENT_CHANGE_HEADER_UNLOAD, mks_lcd_change_header_unload);
  MENU_ITEM(function, MSG_RESUME_PRINT, lcd_sdcard_resume);
  MENU_ITEM(function, MSG_STOP_PRINT, lcd_sdcard_stop);
  END_MENU();
}
    ...
    FORCE_INLINE screenFunc_t ap_message_screen(const AdvancedPauseMessage message) {
      switch (message) {
        ...
        #endif
	#if ENABLED(MKS_CHANGE_FILAMENT)
		case MKS_ADVANCED_PAUSE_MESSAGE:return  mks_lcd_change_filament;
	#endif
    ...
    if (sd_status != lcd_sd_status && lcd_detected()) {
      ...
      if (sd_status) {
        ...
        if (old_sd_status == 2)
          ...
          lcd_setstatusPGM(msg_sd_inserted());
      }
      else {
        card.release();
        //if (old_sd_status != 2) LCD_MESSAGEPGM(MSG_SD_REMOVED);
        if (old_sd_status != 2) lcd_setstatusPGM(msg_sd_removed());
      }
